...
const ACADEMIC_CALENDAR = {
  ...
  "2026-5": ["1(금): 퇴사일", "5(화): 어린이날", "7(목): 스포츠 문화축전 (3학년 졸업앨범 촬영)", "8(금): 1학년 내고장 역사알기, 2·3학년 대학탐방, 퇴사일", "9(토): 과학아카데미 1차", "12(화): 화요일 수업", "14(목)~15(금): 도서관의 날", "15(금): 1인 1기 활동", "20(수)~22(금): 수업공개의 날", "21(목)~29(금): 수강신청 기간", "22(금): 퇴사일", "25(월): 석가탄신일 대체공휴일", "29(금): 1인 1기 활동"],
  ...
  "2026-8": ["8(토): 신입생 3차 전형", "15(토): 광복절", "17(월): 광복절 대체휴일, 입사일", "18(화): 2학기 개학일, 2학기 시작일", "21(금): 1인 1기 활동", "28(금): 퇴사일"],
  "2026-9": ["4(금)~6(일): 과학기술창업캠프", "7(월)~11(금): 수시 원서 접수 기간", "10(목): 과학아카데미 2차", "11(금): 1, 2학년 교육과정설명회, 퇴사일", "16(수): 퇴사일, 교직원 워크숍", "18(금): 1인 1기 활동", "24(목)~27(일): 추석 연휴 (25일 추석)", "29(화): 목요일 대체 수업"],
  "2026-10": ["3(토): 개천절", "5(월): 개천절 대체휴일", "9(금): 한글날", "12(월)~16(금): 1차 지필평가", "16(금): 퇴사일", "17(토): 학술동아리", "23(금): 퇴사일", "26(월): 학술동아리", "30(금): 1인 1기 활동"],
  "2026-11": ["2(월): 월요일 대체 수업, 퇴사일", "3(화): 금요일 대체 수업", "4(수)~6(금): 1, 2학년 자연탐사 / 3학년 입시 프로그램", "6(금): 퇴사일", "11(수)~13(금): 2학기 수업공개의 날", "12(목)~13(금): 독서의 날 행사, 학술동아리", "17(화): 동아리 발표대회", "18(수): 연구활동보고서 제출 (기초, 심화)", "20(금): 퇴사일", "25(수): 연구활동 발표대회 (1, 2학년)", "27(금): 1인 1기 활동"],
  ...
  "2027-2": ["1(월): 퇴사일", "2(화)~13(토): 수강신청", "5(금): 졸업 사정을 위한 영재선정심사위원회", "6(토)~9(화): 설날 연휴 (8일 설날)", "18(목)~19(금): 신학기 교직원 워크숍", "27(토): 졸업식", "28(일): 1, 2학년 2학기 종료일"]
};
...
export default function App() {
  ...
  // [CESAR Helios H-Alpha 이미지 주소] image_e2c2c0.jpg의 소스를 실시간 미러로 반영
  const SUN_IMAGE_URL = "https://cesar.esa.int/sun_monitor/images/latest_halpha.jpg";
  ...
  useEffect(() => {
    const fetchData = () => {
      ...
      fetch(`https://api.openweathermap.org/data/2.5/weather?q=Gwangju&appid=${WEATHER_API_KEY}&units=metric&lang=kr`)
    ...
    <div className="min-h-screen bg-[#020617] text-slate-200 p-8 grid grid-cols-12 gap-8 font-sans overflow-hidden relative">
      ...
      <div className="col-span-3 flex flex-col gap-6 h-full overflow-hidden">
        ...
        <section className="bg-white/5 backdrop-blur-md p-6 rounded-[2rem] border border-white/10 flex items-center justify-around shadow-lg transition-all hover:bg-white/10">
          ...
          <div className="text-right">
            ...
            <p className="text-slate-400 text-sm font-bold uppercase tracking-widest">{weather?.weather?.[0]?.description || "Loading"}</p>
          </div>
        </section>
      ...
      <div className="col-span-5 flex flex-col gap-6 h-full overflow-hidden">
        <div className="grid grid-cols-2 gap-6">
          <section className="bg-black/40 backdrop-blur-md p-4 rounded-[2.5rem] border border-red-500/20 shadow-2xl group relative overflow-hidden">
            <h3 className="text-[10px] font-black text-red-500/80 mb-3 flex items-center gap-2 tracking-widest uppercase"><Sun size={12} /> CESAR Helios H-Alpha</h3>
            <div className="rounded-2xl overflow-hidden aspect-square border border-white/5 bg-slate-900">
              <img src={`${SUN_IMAGE_URL}?t=${Date.now()}`} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000" alt="Sun H-Alpha" />
            </div>
          </section>

          <section className="bg-white/5 backdrop-blur-md p-8 rounded-[2.5rem] border border-white/10 flex flex-col justify-center items-center text-center shadow-xl transition-all hover:bg-white/10">
            ...
            <p className="text-4xl font-black text-yellow-200 tracking-tighter">{getMoonPhase(now).toFixed(1)}<span className="text-sm ml-1 text-slate-500 font-bold italic">d</span></p>
            <p className="text-xs font-bold text-slate-400 mt-2">오늘의 월령</p>
          </section>
        </div>

        {/* [리뉴얼] 학사 일정 통합 박스 레이아웃 */}
        <section className="bg-gradient-to-br from-indigo-500/10 to-transparent backdrop-blur-md p-8 rounded-[3rem] border border-indigo-500/20 flex-1 flex flex-col min-h-0 overflow-hidden shadow-2xl">
          <h3 className="text-2xl font-black text-indigo-300 mb-6 flex items-center gap-3 italic">
            <Bell size={28} className="text-indigo-400" /> {now.getMonth() + 1}월 학사 일정
          </h3>
          <div className="flex-1 overflow-y-auto bg-[#0A0C10]/40 rounded-3xl p-7 border border-white/5 shadow-inner custom-scrollbar relative">
              ...
              <div className="space-y-4 text-left">
                {currentMonthPlans.map((plan, idx) => (
                  <div key={idx} className="flex gap-4 items-start border-b border-white/5 pb-4 last:border-0 last:pb-0 group">
                    <div className="w-1.5 h-1.5 rounded-full bg-indigo-500 mt-2.5 group-hover:scale-150 transition-transform"></div>
                    <p className="text-lg font-bold text-slate-200 leading-relaxed group-hover:text-white transition-colors">{plan}</p>
                  </div>
                ))}
              </div>
            ) : (<p className="text-slate-600 font-bold text-center mt-10 italic">이번 달 일정은 준비 중입니다.</p>)}
          </div>
        </section>
      </div>
      ...
      <aside className="col-span-4 bg-white/5 backdrop-blur-2xl rounded-[3rem] border border-white/10 p-10 flex flex-col shadow-2xl h-full relative">
        ...
        <div className="flex-1 overflow-y-auto space-y-5 pr-3 custom-scrollbar">
          {todos.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center opacity-20 text-slate-500">
              <ListChecks size={80} className="mb-4" /><p className="text-xl font-black italic underline-offset-8 underline decoration-slate-800">Clear Station</p>
            </div>
          ) : (
            todos.map(t => (
        ...
        <footer className="mt-8 pt-8 border-t border-white/5 text-[9px] text-slate-700 font-black text-center tracking-[0.5em] uppercase italic">Smart Station v5.5</footer>
      </aside>
    </div>
  );
}
